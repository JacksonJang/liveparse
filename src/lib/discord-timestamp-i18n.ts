...
export function localizeDiscordDiagnostic(message: string | null | undefined, locale: DiscordUiLocale): string {
  ...
  const spanishMessages: Array<[RegExp, (match: RegExpMatchArray) => string]> = [
    ...
    [/^Enter one timestamp or date per line\.$/, () => 'Introduce un timestamp o una fecha por línea.'],
    [/^Each bulk line is limited to (\d+) characters\.$/, ([limit]) => `Cada línea del lote está limitada a ${limit} caracteres.`],
    [/^Enter one Unix timestamp, Discord tag, or YYYY-MM-DD \[HH:mm\[:ss\]\] value per line\.$/, () => 'Introduce un segundo Unix, una etiqueta de Discord o un valor AAAA-MM-DD [HH:mm[:ss]] por línea.'],
    [/^Enter a real calendar date and time\.$/, () => 'Introduce una fecha y hora de calendario reales.'],
    [/^Enter a valid UTC offset\.$/, () => 'Introduce un desfase UTC válido.'],
    [/^This generator creates timestamps from 1970 onward only\.$/, () => 'Este generador crea solo timestamps de 1970 en adelante.'],
    [/^Bulk generation is limited to \d+ non-empty lines\.$/, () => 'La generación en lote está limitada a 500 líneas no vacías.'],
